    ...
    for epoch in range(max_epochs):
        ...
        if (epoch + 1) % val_interval == 0:
            ...
            with torch.no_grad():
                ...
                savepath = os.path.join(save_models_dir, f"model_ep={pad_zeros_at_front(epoch+1, 4)}.pth")
...
def main(args):
    ...
    save_models_dir = os.path.join(save_models_dir, f'fold{fold}', network, experiment_code)
    os.makedirs(save_models_dir, exist_ok=True)
    
    # save train and valid logs folder
    save_logs_dir = os.path.join(RESULTS_FOLDER,'logs')
    save_logs_dir = os.path.join(save_logs_dir, f'fold{fold}', network, experiment_code)
